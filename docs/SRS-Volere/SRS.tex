...
\begin{itemize}
    \item \textbf{PR-SC1}: The system must avoid false positives to protect students from wrongful accusations. 
    \item \textbf{PR-SC2}: A manual override option for professors must exist to prevent automatic accusations solely based on the model.
\end{itemize}
...
\begin{itemize}
    \item \textbf{PR-C1}: The system should be able to handle batches of inputs without significant delays.
    \item \textbf{PR-C2}: The system should be able to handle batches of inputs without the total processing time exceeding 600 seconds.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{PR-L1}: The system should be designed such that the natural language processing part can be changed in accordance with new research in the field.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{OR-R1: Clear versioning} and change logs to track improvements over time, particularly as new programming languages or detection features are added.
    \item \textbf{OR-R1: Ongoing support}, including the ability to update the plagiarism detector with new models or features as they are developed.
\end{itemize}
...
\begin{itemize}
    \item \textbf{SR-AD1: In Transit Encryption}: The system shall log all user interactions and actions (such as authentication events, code uploads, and plagiarism checks) anonymously, without storing user-uploaded code or sensitive data.
