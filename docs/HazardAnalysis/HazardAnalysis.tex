...
\begin{itemize}
\item \textbf{SR-SAF1: Submission Rate Limitation}: The system shall limit the number of submissions by a parcitular user within a specific time period to prevent server overload.
...
\item \textbf{SR-SAF3: Warning of Potentially Inaccurate Detections}: In cases where the system produces imperfect or potentially erroneous detections, the user shall be warned that the results may be inaccurate.
...
\item \textbf{SR-SAF4: Protection Against Inappropriate Inputs}: The system shall validate all user submissions so that malformed code submissions doesn't compromise system performance or cause system downtime.

Rationale: Malformed inputs could lead to system crashes or incorrect analysis.
