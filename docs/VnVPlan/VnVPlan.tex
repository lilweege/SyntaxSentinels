...
The following table lists team members and their respective roles in the verification and validation process.
...
\begin{table}[h!]
  ...
  \begin{tabular}{|p{5cm}|p{10cm}|}
      ...
      \hline
      Julian Cecchini & Manages the testing of integration points within the system. \\
      \hline
...
\begin{itemize}
    ...
    \item Security of data flows and access control
    \item Component interfaces
    \item Error handling and fault tolerance
\end{itemize}
...
\begin{itemize}
    \item \textbf{Unit Testing}: Each function and module will undergo unit testing to ensure they meet functional requirements.
    \item \textbf{Static Code Analysis}: We will use static analysis tools to verify code quality, adherence to coding standards, and security practices.
...
\begin{itemize}
    \item \textbf{Unit Testing Framework}: A unit testing framework (such as JUnit for Java or Pytest for Python) will be used to automate testing of individual functions and modules.
    \item \textbf{Static Analysis Tools}: Tools such as SonarQube or Valgrind will be used for static analysis to detect code issues, potential security vulnerabilities, and memory leaks.
    \item \textbf{Continuous Integration (CI) Tool}: GitHub Actions or Jenkins will be set up for continuous integration to ensure that tests are automatically run on new code submissions.
    \item \textbf{Code Coverage Tool}: Code coverage tools will track the extent to which the codebase has been tested, ensuring that all critical paths are covered.
    \item \textbf{Linters}: Linters appropriate to the project’s programming language will enforce coding standards, improving code readability and maintainability.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Usability Testing}: We will plan user testing sessions to ensure the software is intuitive and meets usability expectations.
\end{itemize}
