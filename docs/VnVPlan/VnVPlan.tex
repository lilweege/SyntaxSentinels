...
\begin{enumerate}
  ...
  Initial State: System is operational, and spot for initiating analysis is open.
            
  Input: code snippet test set (on the scale of 500) and command to initiate analysis
            
  Output:System presents all appropriate returns with 90\% accuracy and less than 5\%
  ...
  threshold.If accuracy rate is 90\% and false positives are lower than 5\%, the test
   will return a pass. Will be integrated into a CI/CD pipeline and be run periodically
...
\begin{enumerate}
  ...
  Output: all base functions still work with new feature code in place
            
  How test will be performed: When system is given new a feature (as denoted in a 
  pull request or otherwise), all system tests for functional requirements
  are re-applied to ensure no previous functionality has been lost. Can be done 
  via CI/CD pipeline like Jenkins that possess scripts for all functional requirement tests.
\end{enumerate}
...
\begin{enumerate}
  ...
  Input: model code is passed to parser

  Output: parser states model components follow template

  How test will be performed:
  Model Code will be statically checked
...
changes to its components). Will be integrated into a CI/CD pipeline and run
...
checkpoint file or a return value from the training script. Will be integrated into a CI/CD pipeline 
...
\begin{enumerate}
  ...
  Input: not given into system, appropriately dimensioned vector into model component/layer
