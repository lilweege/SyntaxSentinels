...
Initial State: The system is active, zero or more existing snippets are currently uploaded, and the system is set up to receive code snippet uploads.
					
Input: Code snippet file(s).
					
Output: The system signifies that the input was received and continues activity without error, awaiting further action directives (such as uploading more files or initiating analysis).

Test Case Derivation: The system will continually receive code snippets from the user as part of a necessary step to initiate plagiarism analysis.
					
How the test will be performed: A script will open the system in the appropriate spot for
receiving code and will upload files in varying amounts, one batch at a time, while inspecting for failures during the process. The script will be integrated into the CI/CD
...
This section applies to all FRs involving the immediate output of plagiarism analysis. This
...
Initial State: The system is active, two or more code snippets have been uploaded, and the system is set up to initiate analysis.
					
Input: Command to initiate analysis (button, enter, etc.).
					
Output: The system presents a list of similarity scores for all code snippet pairings
along with corresponding threshold scores. Any code snippet pairing that
has exceeded its threshold has been flagged. The system remains active,
awaiting further action directives.
...
scores should be generated for every pairing of code snippets, as plagiarism is a relative assessment between one piece of code and another. The system must be able to pass the received code snippets to this algorithm and use its results to flag any pairing that exceeds its threshold before returning from
its analysis state to the user for further interaction.

How the test will be performed: A script will open the system's spot to initiate analysis with two or more code snippets already inserted into the system, and a command to start the analysis will be issued. Errors will be inspected for until similarity scores and thresholds have been presented. At this point, code flaggings should also be available for verification (type: dynamic, functional).
					
\end{enumerate}

\subsubsection{Guide Documentation Generation Tests}

This section applies to all FRs involving guide documentation generation for the user. This currently covers FR-3 in subsection 9.1 of the SRS \citep{SRS}.
...
Initial State: The system is active, and the system is set up to generate guide documentation.
					
Input: Command for documentation presentation (button, enter, etc.).
					
Output: The system generates documentation in a viewable medium, such as text or a PDF file, and remains active, awaiting further action directives.

Test Case Derivation: The system possesses guide documentation that it should be capable of transferring to the user to provide guidance on use cases of the system when prompted.

How the test will be performed: A script will open the system's spot for generating guide documentation, and a command for generating the guide documentation will be issued, all while inspecting for errors. It will check for missing documentation at the end (type: dynamic, functional).
					
\end{enumerate}

\subsubsection{Report Documentation Generation Tests}

This section applies to all FRs involving analysis report documentation generation for the user. This currently covers FR-6 of subsection 9.1 of the SRS \citep{SRS}.
...
Initial State: The system is active, the most recent plagiarism analysis has been completed since system activation, and its results are available. The system is set up for report generation.
					
Input: Command for report documentation generation (button, enter, etc.).
					
Output: The system provides report documentation in a viewable medium, such as text or a PDF file, and remains active, awaiting further action directives.

Test Case Derivation: The system should possess the ability to aggregate the results of the most recent plagiarism analysis and insert them into a template that can summarize all findings for the user, which will become the report document provided.

How the test will be performed: A script will have the system conduct a plagiarism analysis to provide inputs for the report generation, simulating what will have to happen for a user to unlock report generation. If successful, it will proceed to open the spot for generating a report, where the command for generating the report will be issued, all while inspecting for errors during the process. It will check for missing documentation at the end (type: dynamic, functional).
					
\end{enumerate}

\subsubsection{Account Creation Tests}

This section applies to all FRs involving creating an account within the system. This currently covers FR-7 of subsection 9.1 of the SRS \citep{SRS}.
...
Initial State: The system is active, and the system is set up for account creation.
					
Input: Command for account creation (button, enter, etc.), along with account user email and password, ensuring the email is not associated with any existing account.
					
Output: The system indicates that the account has been successfully created for logging in (evidenced by the existence of the email within the created account list) and remains active, awaiting further action directives.

Test Case Derivation: The system should be able to assess that a set of account credentials is not yet within the system and proceed to add this set to the set of accounts that can be logged in with.

How the test will be performed: A script will open the system's spot for account creation, provide an email and password not associated with any existing account in the appropriate area, and issue the command for account creation, all while inspecting for any failure modes during the process (type: dynamic, functional). The created account will be inspected at the end.
...
Input: Command for account creation (button, enter, etc.), along with account user email and password, ensuring the email is associated with an existing account.
					
Output: The system notifies that account creation was not possible for logging in and remains active, awaiting further action directives.

Test Case Derivation: A set of pre-existing account credentials should not be possible to use for account creation. Otherwise, account creation is arbitrary and not truly provided by the system, as any set of account credentials would not be tied to any particular account.

How the test will be performed: A script will open the system's spot for account creation, provide an email and password associated with an existing account in the appropriate area, and issue the command for account creation, all while inspecting for any failure modes during the process (type: dynamic, functional).
					
\end{enumerate}

\subsubsection{Account Login Tests}

This section applies to all FRs involving logging into an account within the system. This currently covers FR-8 of subsection 9.1 of the SRS \citep{SRS}.
...
Initial State: The system is active, and the system is set up for account login.
					
Input: Command for account login (button, enter, etc.), along with account user email and password, ensuring the email is associated with an existing account.
					
Output: The system notifies that the account login was successful and remains active, awaiting further action directives.

Test Case Derivation: The system should be able to validate a set of pre-existing account credentials to facilitate login.

How the test will be performed: A script will open the system's spot for account login, provide the email and password associated with the existing account in the appropriate area, and issue the command for account login, all while inspecting for any failure modes during the process (type: dynamic, functional).

\item{test-FR-8\\}

Control: Automatic.
					
Initial State: The system is active and is set up for account login.
					
Input: Command for account login (button, enter, etc.), along with account user email and password, ensuring the email is not associated with any existing account.
					
Output: The system notifies that the account login was not successful and remains active, awaiting further action directives.

Test Case Derivation: A set of account credentials not associated with an existing account will fail

 to allow login, as the system should determine there is no account to validate against.

How the test will be performed: A script will open the system's spot for account login, provide an email and password not associated with any existing account in the appropriate area, and issue the command for account login, all while inspecting for any failure modes during the process (type: dynamic, functional).
					
\end{enumerate}

\subsubsection{Result Email Tests}

This section applies to all FRs involving emailing the results of plagiarism analysis to users. This currently covers FR-9 of subsection 9.1 of the SRS \citep{SRS}.
...
Initial State: The system is active, the most recent plagiarism analysis has been completed since system activation, its results are available, and the system is set up for emailing results.
					
Input: Command for emailing results (button, enter, etc.) and email to receive results.
					
Output: The system notifies that the email has been sent and remains active, awaiting further action directives. Externally, the specified email should contain a .zip file with the results of the recent analysis.

Test Case Derivation: The system should possess the ability to condense results into a .zip file upon demand, then proceed to send this file by email.

How the test will be performed: A script will have the system conduct a plagiarism analysis to provide inputs for emailing, simulating the steps needed for a user to enable email initiation. If successful, the script will proceed to the system's spot for emailing results, issue the command for sending the email alongside an email address for receiving the results, and inspect for errors during the process. The reception of the .zip file will be assessed at the end when the system is awaiting action directives (type: dynamic, functional).
					
\end{enumerate}

\subsubsection{Result Visualization Tests}

This section applies to all FRs involving visualizing plagiarism analysis results provided by the user in a .zip file. This currently covers FR-10 of subsection 9.1 of the SRS \citep{SRS}.
...
Initial State: The system is active and is set up for inserting .zip files containing results to produce visualization.
					
Input: Command for visualization (button, enter, etc.) and .zip file containing results.
					
Output: The system provides visualization in a viewable medium, such as an image or file, that corresponds to the results in the given .zip file.

Test Case Derivation: The system should possess the ability to parse a .zip file for relevant contents, pass the data into an internal visualization template, and then transfer the generated visualization to the user.

How the test will be performed: A script will open the system's spot for inserting a .zip file to produce a visualization, issue the command for visualization alongside the .zip file containing results, and inspect for errors during the process. At the end, the script will check for missing parts of the visualization.
